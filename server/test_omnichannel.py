...
def test_omnichannel():
    ...
    # 1. Test Phishing (Debe ser bloqueado)
    print("\n1. Probando Bloqueo de Phishing...")
    phishing_payload = {
    ...
    try:
        ...
        if r.status_code == 400 and "Bloqueado por seguridad" in r.json().get("error", ""):
    ...
    try:
        ...
        if r.status_code == 200:
            print(f"✅ PASS: Ticket procesado. Churn Score: {data.get('churn_score')}")
            if data.get('churn_score') > 70:
    ...
    try:
        ...
        if len(tickets) > 0:
            ...
            print(f"   -> Último ticket: {last_ticket['client_id']} - Score: {last_ticket['churn_score']}")
    except Exception as e:
        print(f"❌ ERROR: {e}")
